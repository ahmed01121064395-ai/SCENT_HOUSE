...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    console.log(`[Paymob Callback GET] Redirect details: success=${success}, merchant_order_id=${merchantOrderId}, order=${orderId}`);

    // Read the app base URL from environment variables, fallback to window origin on client or standard domain
    const baseUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://scent-house.vercel.app';
    ...
    if (success === 'true' && merchantOrderId) {
      ...
      return NextResponse.redirect(`${baseUrl}/confirmation?orderId=${merchantOrderId}&success=true`);
    } else {
